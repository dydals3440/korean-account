...
**라이브러리 0.2.0 기준** · 데이터 출처: 금융결제원 [_CMS 참가기관별 계좌번호체계_](https://www.cmsedi.or.kr/cms/board/workdata/cms) PDF 전체 22쪽 (기준일 2026.05.08)

korean-account 의 전체 레퍼런스다. 앞부분 (§ 0–6) 은 PDF 를 기관 단위로 전사한 **데이터 레퍼런스**, 뒷부분 (Appendix A–E) 은 **라이브러리 레퍼런스**. 빠른 시작·설치는 [README](./README.md) 에 있다.

| 찾는 것                                              | 위치                                                                  |
| ---------------------------------------------------- | --------------------------------------------------------------------- |
| 과목코드 위치 4패턴 — 데이터 읽는 법                 | [§ 0](#0-식별-위치-패턴-개발자-핵심)                                  |
| 기관별 계좌번호 체계 전수 (은행 § 1 · 증권 § 2)      | [§ 1](#1-은행-banks) · [§ 2](#2-증권사)                               |
| CMS 코드표 · 개발자 체크리스트 · 최근 변경           | [§ 3](#3-cms-참가기관-코드표-전체) – [§ 5](#5-최근-추가변경-20242026) |
| CMS 코드 vs 표준은행코드 — 두 namespace              | [§ 6](#6-kftc-표준은행코드-vs-cms-참가기관-코드)                      |
| 타입 · 스코어링 · tie-break `prevalence` · 분기 규칙 | [Appendix A](#appendix-a-api-레퍼런스)                                |
| 코어가 어디까지 책임지는가 — PDF vs 컨슈머           | [Appendix B](#appendix-b-pdf-vs-컨슈머-책임-모델)                     |
| 폼 검증 · React · TanStack · shadcn/ui 레시피        | [Appendix C](#appendix-c-recipes)                                     |
| `extend` / `remove` · 보강 카탈로그 8건 · 선택자     | [Appendix D](#appendix-d-확장-mechanics)                              |
| **0.1.x → 0.2.0 마이그레이션** — rename 전체 대응표  | [Appendix E](#appendix-e-마이그레이션-가이드--01x--020)               |

> **0.1.x 에서 업그레이드한다면 Appendix E 부터.** 0.1.x 의 옛 API 이름은 이 문서 본문에 더 이상 등장하지 않는다 — 새 이름과의 전체 대응표가 Appendix E 에 있다.

---

# 데이터 레퍼런스 — CMS 참가기관별 계좌번호 체계

PDF 표를 기관 단위로 전수 전사한 부분이다. 라이브러리 레지스트리 (`src/registry/`) 의 1차 진실원본이며, 표의 행 하나가 코드의 `AccountPattern` 하나에 대응한다.
